<template>
  <div class="main-content">
    <div class="page-header">
      <h3 class="page-title">New Milestone</h3>
    </div>
    <div class="row">
          ...
          <div class="card-body">
             ...
             <div class="form-group">
              ...
              <div class="form-row">
                  ...
                  <label for="portfolio_id">Select Project</label>
                    <select v-model="project_id" class="form-control">
                  <option v-for="(item,index) in project" :key="index" :value="item.id">{{item.name}}</option>
                </select>
                </div>
                  <div class="form-group col-md-6">
...
import Datepicker from 'vuejs-datepicker'
    export default {
      ...
      axios.post('/api/create-milestone', 
      formData,
     
      {headers: {
      'Content-type': 'application/x-www-form-urlencoded',
    }}).then(r => console.log(r.data)).catch(err => console.log(err))
    this.$router.push({path:"/admin/page/milestone"})
     window.toastr['success']('Milestone Added', 'Success')
    },
